...
public class CubeCreatorCategory extends BlankRecipeCategory<CubeCreatorRecipeWrapper> {
		...
		public void setRecipe(@Nonnull IRecipeLayout recipeLayout, @Nonnull CubeCreatorRecipeWrapper recipe) {
				IGuiItemStackGroup stacks = recipeLayout.getItemStacks(); if (recipe.getInputs().size() > 0) {
						...
						stacks.init(2, true, 0, 18); stacks.setFromRecipe(2, recipe.getInputs().get(2));
				} if (recipe.getInputs().size() > 3) {
						stacks.init(3, true, 72, 18); stacks.setFromRecipe(3, recipe.getInputs().get(3));
				} if (recipe.getInputs().size() > 4) {
						stacks.init(4, true, 0, 54); stacks.setFromRecipe(4, recipe.getInputs().get(4));
				} if (recipe.getInputs().size() > 5) {
						stacks.init(5, true, 72, 54); stacks.setFromRecipe(5, recipe.getInputs().get(5));
				} if (recipe.getInputs().size() > 6) {
						stacks.init(6, true, 18, 72); stacks.setFromRecipe(6, recipe.getInputs().get(6));
				} if (recipe.getInputs().size() > 7) {
						stacks.init(7, true, 54, 72); stacks.setFromRecipe(7, recipe.getInputs().get(7));
				} stacks.init(9, true, 36, 36); stacks.setFromRecipe(9, recipe.getOutputs().get(0));
		}
}
