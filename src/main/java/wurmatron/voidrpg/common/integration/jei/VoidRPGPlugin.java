...
public class VoidRPGPlugin extends BlankModPlugin {
		...
		public void register(@Nonnull IModRegistry registry) {
				registry.addAdvancedGuiHandlers(new IAdvancedGuiHandler<GuiCubeCreator>() {
						...
						public List<Rectangle> getGuiExtraAreas(GuiCubeCreator gui) {
								...
								list.add(new Rectangle((gui.width - 256) / 2, (gui.height - 212) / 2, 256, 212)); return list;
						}
				}); registry.addRecipeCategories(new CubeCreatorCategory(registry.getJeiHelpers().getGuiHelper()));
				registry.addRecipeHandlers(new CubeCreatorRecipeHandler()); registry.addRecipes(getCubeCreatorRecipes());
		...
		private String getValidArmorTypes(ICube cube) {
				if (cube != null) {
						String temp = ""; if (cube.getSupportedItem(EntityEquipmentSlot.HEAD, null)) temp += "Helmet, ";
						if (cube.getSupportedItem(EntityEquipmentSlot.LEGS, null)) temp += "Leggings, ";
						if (cube.getSupportedItem(EntityEquipmentSlot.CHEST, null)) temp += "Chestplate, ";
						if (cube.getSupportedItem(EntityEquipmentSlot.FEET, null)) temp += "Boots, ";
						if (temp.endsWith(",")) return temp.substring(0, temp.lastIndexOf(",") - 1); return temp;
				} return "none";
		}
}
