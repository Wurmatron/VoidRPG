...
public class TileCubeCreator extends TileEntity implements IInventory, ITickable {
		...
		public ItemStack getStackInSlot(int index) {
				if (index < getSizeInventory()) return inventory[index]; return null;
		}

		@Override
		public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
				super.writeToNBT(nbt); NBTTagList list = new NBTTagList(); for (int i = 0; i < this.getSizeInventory(); ++i) {
						if (this.getStackInSlot(i) != null) {
								NBTTagCompound stackTag = new NBTTagCompound(); stackTag.setByte(NBT.SLOT, (byte) i);
								this.getStackInSlot(i).writeToNBT(stackTag); list.appendTag(stackTag);
						}
				} nbt.setInteger(NBT.TIMER, timer); nbt.setTag(NBT.INVENTORY, list); return nbt;
		}


		@Override
		public void readFromNBT(NBTTagCompound nbt) {
				super.readFromNBT(nbt); NBTTagList list = nbt.getTagList(NBT.INVENTORY, 10);
				for (int i = 0; i < list.tagCount(); ++i) {
						NBTTagCompound stackTag = list.getCompoundTagAt(i); int slot = stackTag.getByte(NBT.SLOT) & 255;
		...
		public ItemStack decrStackSize(int index, int count) {
				if (getStackInSlot(index) != null) {
						ItemStack stack; if (getStackInSlot(index).stackSize <= count) {
								stack = getStackInSlot(index); setInventorySlotContents(index, null); markDirty(); return stack;
						} else {
								stack = getStackInSlot(index).splitStack(count);
								if (getStackInSlot(index).stackSize <= 0) setInventorySlotContents(index, null);
								else setInventorySlotContents(index, getStackInSlot(index)); markDirty(); return stack;
						}
				} return null;
		}
		...
		public ItemStack removeStackFromSlot(int index) {
				if (index < getSizeInventory()) setInventorySlotContents(index, null); return null;
		}

		@Override
		public void setInventorySlotContents(int index, @Nullable ItemStack stack) {
				if (index < getSizeInventory()) {
						inventory[index] = stack; markDirty();
				}
		}
		...
		private void hasValidRecipe() {
				for (ICubeCreatorRecipe recipe : CubeCreatorRecipeHandler.getRecipes()) {
						boolean hasItems = true; if (recipe.getInputs() != null && recipe.getOutputCube() != null) {
								for (ItemStack stack : recipe.getInputs()) {
										if (!hasStack(stack)) hasItems = false; if (hasItems && activeRecipe == null) setActiveRecipe(recipe);
								}
						}
				}
		}
		...
		private boolean hasStack(ItemStack stack) {
				int amount = 0; for (int slot = 1; slot <= 8; slot++) {
						if (getStackInSlot(slot) != null && StackHelper.areItemsEqual(stack, getStackInSlot(slot))) return true;
		...
		private void addOutput(ItemStack stack) {
				if (stack != null && getStackInSlot(0) == null) {
						...
						if (stack.getTagCompound() != null) item.setTagCompound(stack.getTagCompound());
		...
		private void setActiveRecipe(ICubeCreatorRecipe recipe) {
				if (recipe != null) {
						timer = recipe.getTimeInTicks(); consumeRecipeItems(recipe); activeRecipe = recipe;
				}
		}
}
