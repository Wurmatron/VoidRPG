...
public class CubeJson implements ICube {
		...
		public CubeJson(String name, String block, ResourceLocation texture, double weight, int maxDurability, int complexity, int maxAmount, String[] supported, String description, double protection) {
				this.name = name; if (block != null && block.length() > 0 && block.contains(":")) this.block = block;
				else LogHandler.debug("Invalid block for json cube found " + name); this.texture = texture; this.weight = weight;
				this.maxAmount = maxAmount; this.complexity = complexity; this.maxDurability = maxDurability;
				this.supported = supported; this.description = description; this.protection = protection;
		}
