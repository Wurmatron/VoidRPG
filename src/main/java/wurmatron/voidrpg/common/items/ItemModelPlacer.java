...
public class ItemModelPlacer extends Item {
		...
		public ItemModelPlacer() {
				setCreativeTab(VoidRPG.tabVoidRPG); setUnlocalizedName("modelPlacer"); setHasSubtypes(true); setMaxStackSize(1);
		}
		...
		public EnumActionResult onItemUse(ItemStack stack, EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
				if (!world.isRemote) {
						switch (stack.getItemDamage()) {
								case (0): {
										ArrayList<Vec3i> modelHead = new ArrayList<>(); for (int x = 0; x < 15; x++)
												for (int y = 0; y < 15; y++)
														for (int z = 0; z < 15; z++)
										...
										ArrayList<Vec3i> modelArm = new ArrayList<>(); for (int x = 6; x < 10; x++)
												for (int y = 0; y < 12; y++)
														for (int z = 6; z < 10; z++)
										...
										ArrayList<Vec3i> modelLegs = new ArrayList<>(); for (int x = 4; x < 12; x++)
												for (int y = 0; y < 9; y++)
														for (int z = 6; z < 10; z++)
																modelLegs.add(new Vec3i(x, y, z));
										BitHelper.createBaseArmorBlock(modelLegs.toArray(new Vec3i[0]), world, pos); return EnumActionResult.SUCCESS;
								} case (3): {
										ArrayList<Vec3i> modelBoots = new ArrayList<>(); for (int x = 4; x < 12; x++)
												for (int y = 0; y < 5; y++)
														for (int z = 6; z < 10; z++)
																modelBoots.add(new Vec3i(x, y, z));
										BitHelper.createBaseArmorBlock(modelBoots.toArray(new Vec3i[0]), world, pos); return EnumActionResult.SUCCESS;
		...
		public static String getNameFromMeta(int meta) {
				switch (meta) {
						case (0): return "Helmet"; case (1): return "ChestPlate"; case (2): return "Leggings"; case (3): return "Boots";
						default: return "Invaid";
				}
		}
}
