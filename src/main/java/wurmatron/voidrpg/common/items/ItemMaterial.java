...
public class ItemMaterial extends Item {
		...
		public ItemMaterial() {
				setCreativeTab(VoidRPG.tabVoidRPG); setHasSubtypes(true); setUnlocalizedName("itemMaterial");
		...
		public static ItemStack createMaterial(String name, int size) {
				for (int s = 0; s <= VoidRPGItems.materials.length; s++)
						if (name.equals(VoidRPGItems.materials[s])) return new ItemStack(VoidRPGItems.itemMaterial, size, s);
				return new ItemStack(VoidRPGItems.itemMaterial, size, -1);
		}

		public static ItemStack createMaterial(String name) {
				for (int s = 0; s <= VoidRPGItems.materials.length; s++)
						if (name.equals(VoidRPGItems.materials[s])) return new ItemStack(VoidRPGItems.itemMaterial, 1, s);
		...
		public String getUnlocalizedName(ItemStack stack) {
				if (stack.getItemDamage() < VoidRPGItems.materials.length)
						return "item." + VoidRPGItems.materials[stack.getItemDamage()]; return "item.material.error.name";
		}
}
