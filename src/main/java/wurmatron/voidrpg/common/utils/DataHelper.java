...
public class DataHelper {
		...
		public static CubeData damageCube(CubeData data, int damage) {
				if (Math.abs(damage) > 0) return new CubeData(data.cube, data.xPos, data.yPos, data.zPos, data.damage + damage);
				return data;
		}
		...
		public static CubeData[] rotateClockwise(CubeData[] cubes) {
				CubeData[] data = new CubeData[cubes.length]; for (int s = 0; s < cubes.length; s++)
		...
		public static final ItemStack createHelmetFromData(CubeData[] helmetData) {
				ItemStack stack = new ItemStack(VoidRPGItems.armorHelmet, 1, 0); NBTTagCompound tag = new NBTTagCompound();
				if (helmetData != null && helmetData.length > 0) {
						NBTTagCompound[] nbt = BitHelper.convertCubesToNBT(helmetData); for (int i = 0; i < nbt.length; i++)
		...
		private static final NBTTagCompound[] convertDataToNBT(CubeData[]... cubeData) {
				NBTTagCompound[] nbt = new NBTTagCompound[cubeData.length]; for (int index = 0; index < cubeData.length; index++) {
						NBTTagCompound tag = new NBTTagCompound(); if (cubeData[index] != null && cubeData[index].length > 0) {
								NBTTagCompound[] d = BitHelper.convertCubesToNBT(cubeData[index]); for (int i = 0; i < d.length; i++)
										tag.setTag(Integer.toString(i), nbt[i]); nbt[index] = tag;
						}
				} return nbt;
		}
		...
		private static final NBTTagCompound[] getNBTDataFromStack(ItemStack stack) {
				if (stack != null && stack.getTagCompound() != null) {
						...
						for (int i = 0; i < stackData.length; i++) {
								NBTTagCompound temp = stackData[i]; for (int s = 0; s <= temp.getSize(); s++)
		...
		private static final NBTTagCompound[] getDataFromNBTData(NBTTagCompound nbt) {
				NBTTagCompound[] data = new NBTTagCompound[nbt.getSize()]; for (int i = 0; i < nbt.getSize(); i++)
						data[i] = nbt.getCompoundTag(Integer.toString(i)); return data;
		}
		...
		public static CubeData[] getDataFromStack(ItemStack stack, String type) {
				ArrayList<CubeData> data = new ArrayList<>(); if (type.length() <= 0) {
		...
		public static int getMaxDurability(ItemStack stack, boolean update) {
				boolean temp = update; if (stack != null && stack.hasTagCompound()) {
						if (!stack.getTagCompound().hasKey(NBT.MAX_DURABILITY)) temp = true; if (temp) {
								CubeData[] cubes = getDataFromStack(stack); int total = 0; for (CubeData cube : cubes)
										if (cube != null && cube.cube != null) total += cube.cube.getMaxDurability();
		...
		public static int getComplexity(ItemStack stack, boolean update) {
				boolean temp = update; if (stack != null && stack.hasTagCompound()) {
						if (!stack.getTagCompound().hasKey(NBT.COMPLEXITY)) temp = true; if (temp) {
								CubeData[] cubes = getDataFromStack(stack); int complexity = 0; for (CubeData cube : cubes)
										if (cube != null && cube.cube != null) complexity += cube.cube.getComplexity();
