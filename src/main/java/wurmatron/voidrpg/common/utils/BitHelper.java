...
public class BitHelper {
		...
		public static boolean hasValidModel(World world, BlockPos pos, Vec3i[] model) {
				...
				if (world != null && pos != null && model != null && model.length > 0 && api.isBlockChiseled(world, pos) && !world.isRemote) {
						try {
								IBitAccess bit = api.getBitAccess(world, pos); for (Vec3i vec : model) {
		...
		public static CubeData[] createDataFromModel(World world, BlockPos pos) {
				ArrayList<CubeData> data = new ArrayList<>(); if (!world.isRemote && api.isBlockChiseled(world, pos)) try {
						IBitAccess bit = api.getBitAccess(world, pos); for (int x = 16; x >= 0; x--)
								for (int y = 16; y >= 0; y--)
										for (int z = 16; z >= 0; z--)
		...
		public static NBTTagCompound[] convertCubesToNBT(CubeData[] data) {
				NBTTagCompound defaultCubes          = new NBTTagCompound(); NBTTagCompound specialCubes = new NBTTagCompound();
				NBTTagCompound energyStorageCubes    = new NBTTagCompound();
				NBTTagCompound energyProductionCubes = new NBTTagCompound(); for (CubeData cube : data) {
						if (cube != null && cube.cube != null) if (cube.cube.hasEffects()) {
								NBTTagCompound nbt = convertCubeDataToNBT(cube);
								if (!nbt.hasNoTags()) specialCubes.setTag(Integer.toString(specialCubes.getSize() + 1), nbt);
						} else if (cube.cube instanceof IEnergy) {
								IEnergy energy = (IEnergy) cube.cube; if (energy.getStorage() > 0) {
										NBTTagCompound nbt = convertCubeDataToNBT(cube);
										if (!nbt.hasNoTags()) energyStorageCubes.setTag(Integer.toString(energyStorageCubes.getSize() + 1), nbt);
								} else if (energy.getProductionAmount() > 0) {
										NBTTagCompound nbt = convertCubeDataToNBT(cube); if (!nbt.hasNoTags())
												energyProductionCubes.setTag(Integer.toString(energyProductionCubes.getSize() + 1), nbt);
								}
						} else {
								NBTTagCompound nbt = convertCubeDataToNBT(cube);
								if (!nbt.hasNoTags()) defaultCubes.setTag(Integer.toString(defaultCubes.getSize() + 1), nbt);
						}
		...
		public static NBTTagCompound convertCubeDataToNBT(CubeData data) {
				NBTTagCompound nbt = new NBTTagCompound(); nbt.setByte(NBT.CUBE_ID, CubeRegistry.getIDForCube(data.cube));
				nbt.setByte(NBT.CUBE_X, (byte) data.xPos); nbt.setByte(NBT.CUBE_Y, (byte) data.yPos);
				nbt.setByte(NBT.CUBE_Z, (byte) data.zPos); nbt.setInteger(NBT.CUBE_DAMAGE, data.damage);
				if (nbt.getByte(NBT.CUBE_ID) != -1) return nbt; return new NBTTagCompound();
		}
		...
		public static ModelRenderer createModelRenderer(ModelBiped base, CubeData data) {
				ModelRenderer model = new ModelRenderer(base) {
						...
						public void render(float scale) {
								Minecraft.getMinecraft().renderEngine.bindTexture(data.cube.getTexture()); super.render(scale);
								Minecraft.getMinecraft().renderEngine.bindTexture(new ResourceLocation(Global.MODID, "textures/armor/armor.png"));
						}
				}; model.addBox(data.xPos, data.yPos, data.zPos, 1, 1, 1, 0.00001f); return model;
		}

		private static Vec3i[] inverseModel(Vec3i[] model) {
				ArrayList<Vec3i> vec = new ArrayList<>(); Collections.addAll(vec, model);
				ArrayList<Vec3i> output = new ArrayList<>(); for (int x = 0; x < 15; x++)
						for (int y = 0; y < 15; y++)
								for (int z = 0; z < 15; z++)
										if (!vec.contains(new Vec3i(x, y, z))) output.add(new Vec3i(x, y, z)); return output.toArray(new Vec3i[0]);
		}

		public static void createBaseArmorBlock(Vec3i[] model, World world, BlockPos pos) {
				if (!world.isRemote) {
						world.setBlockState(pos, VoidRPGBlocks.bodyBlock.getDefaultState()); try {
								IBitAccess block = api.getBitAccess(world, pos); createModelFromData(block, model);
						} catch (APIExceptions.CannotBeChiseled e) {
				...
				if (block != null && data != null && data.length > 0) {
						try {
								for (int x = 0; x <= 15; x++) {
										for (int y = 0; y <= 15; y++)
												for (int z = 0; z <= 15; z++) {
														ArrayList<Boolean> temp = new ArrayList<>(); for (Vec3i vec : data) {
																if (!vec.equals(new Vec3i(x, y, z))) temp.add(false);
																else temp.add(true);
														} if (!temp.contains(true)) block.setBitAt(x, y, z, airBrush);
												}
